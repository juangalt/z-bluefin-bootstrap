...
@test "set-hostname: skips Tailscale step when tailscale not installed" {
  mock_cmd_capture hostnamectl 0
  # Override have() so tailscale reports missing while other lookups still work.
  have() { [[ "$1" == "tailscale" ]] && return 1; command -v "$1" &>/dev/null; }
  run cmd_set_hostname "my-new-host"
  assert_success
  assert_output --partial "Hostname set to 'my-new-host'"
